...
export default async function RecettesPage() {
  ...
  const [{ data: recettes }, { data: produits }, { data: vins }] = await Promise.all([
    (supabase as any)
      .from('recettes')
      .select('*, recette_ingredients!recette_ingredients_recette_id_fkey(id, quantite, unite, cout_unitaire, cout_total, produits(nom), vins(nom))')
      .eq('actif', true)
      .eq('organization_id', orgUUID)
      .order('type').order('nom'),
    (supabase as any)
      .from('produits')
      .select('id, nom, categorie, unite, prix_unitaire_ht')
      .eq('actif', true)
      .eq('organization_id', orgUUID)
      .order('nom'),
    (supabase as any)
