...
interface CommandeData {
  ...
  lignes: { produit_nom: string; unite: string; quantite: number; prix_unitaire: number }[]
  total_ht: number
  note?: string | null
}

export function generateBonCommandePDF(data: CommandeData): Uint8Array {
  const doc = new jsPDF()
  ...
  if (data.date_livraison_prevue) {
    doc.text(`Livraison souhaitee : ${data.date_livraison_prevue}`, 105, 42, { align: 'center' })
  }
